...
frappe.ui.form.on('Stock Checker', {
	setup: (frm) => {
		frm.set_query('item', () => {
	...
	get_stock_and_item_details: (frm) => {
		if (!(frm.doc.warehouse && frm.doc.date)) {
			...
			frappe.call({
				method: 'isupport.istock.doctype.stock_checker.stock_checker.get_stock_item_details',
				args: filters,
				callback: (r) => {
					if (r.message) {
						let fields = ['item', 'qty', 'value', 'image', 'selling_price', 'last_buying_rate'];
